...
> 📦 **This pack: version `2026-07-03` (built 3 July 2026).** To check you're on the latest, compare this against the newest pack in the REDACTED — or just run `Update Agent OS.command`.
>
...
- **Every install is now truly yours** — fixed a bug where a fresh setup acted like the creator's (called you "REDACTED", referenced his projects/company/voice). Now it's clean + personal; set your name with `"userName"` in `config.json`.
