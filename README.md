...
- **Hermes Outreach** — lead-gen and email outreach built into the Hermes tab: Hunter.io to find, verify, and size contacts; Firecrawl for enrichment; Apollo.io for sourcing. Sends go out through a real mailbox (Himalaya over OAuth2 IMAP/SMTP, or Gmail), with built-in opt-out handling on every message.
- **Radar** — a 24/7 AI-news scanner that reads X (Twitter) through Hermes's built-in search, surfaces trending AI stories, drafts quote-tweet responses, and can turn a story into a full SEO article — with the source tweet embedded — and publish it straight to WordPress.
- **Missions** — the live task-graph dashboard for everything the kernel dispatches: an SSE-powered board, verifier evidence, log tails, and a full event-replay timeline. (See "Architecture" below for how the underlying routing and verification actually work.)
- **Goals** — a lightweight goal tracker backed by the same Obsidian vault the agents use for memory, so objectives live alongside everything else the system already knows rather than in a separate tool.
- **Game Studio** — commission a game from a plain-text prompt; a game-development agent builds a playable game and it lands in a gallery you can open directly in the dashboard.
- **OpenMontage** — prompt-to-video: type a description, get a cinematic video, generated by an open-source agentic video pipeline wired into the same dashboard as the business tooling.
- **Memory** — a direct view into the Obsidian vault the agents read from and write to, plus a "Memory Galaxy" view for browsing it spatially instead of as a flat file tree.
