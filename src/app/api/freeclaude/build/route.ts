...
function localModel(): string {
  ...
  // MODEL is a cloud slug (e.g. opencode_go/glm-5.2 for the chat proxy), not an
  // ollama model — so the on-device build/fallback uses the pinned LOCAL_MODEL
  // (an installed model), never the un-pulled 12B default that 404s on this machine.
