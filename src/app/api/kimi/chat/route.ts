...
export async function POST(req: Request) {
  ...
  // kimi-cli ≥1.48 requires explicit --print for non-interactive output formats
  // (`-p` alone no longer implies print mode — "Output format is only supported for
  // print UI"). RE-APPLY-AFTER-UPDATE item.
  const args = ["--print", "-p", fullPrompt, "--output-format", "stream-json"];
