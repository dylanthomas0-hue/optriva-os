...
// Env-overridable so this tab can route via the local Headroom gateway → OpenCode Go
// glm-5.2 (GLM_CHAT_ENDPOINT + GLM_CHAT_KEY set in start-dashboard.command) — the
// z.ai key on this machine is empty. Stock default kept. RE-APPLY-AFTER-UPDATE item.
const ENDPOINT = process.env.GLM_CHAT_ENDPOINT || "https://api.z.ai/api/coding/paas/v4/chat/completions";
const MODEL = process.env.GLM_CHAT_MODEL || "glm-5.2";

function glmKey(): string | null {
  const e = process.env.GLM_CHAT_KEY || process.env.GLM_API_KEY || process.env.ZAI_API_KEY || process.env.Z_AI_API_KEY;
  if (e) return e.trim();
