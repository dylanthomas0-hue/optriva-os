...
export async function POST(req: Request) {
  ...
  // Inject the OpenAI key into codex's own spawn env only (from ~/.hermes/.env).
  // Deliberately NOT a global dashboard export: the kimi CLI's OpenAI-compatible
  // provider prefers env keys over its config.toml and would 401 against its own
  // upstream. RE-APPLY-AFTER-UPDATE item.
  const extraEnv: Record<string, string> = {};
  if (!process.env.OPENAI_API_KEY) {
    try {
      const henv = fs.readFileSync(path.join(os.homedir(), ".hermes", ".env"), "utf8");
      const m = henv.match(/^OPENAI_API_KEY=(.+)$/m);
      if (m) extraEnv.OPENAI_API_KEY = m[1].trim();
    } catch { /* no key file — codex falls back to its own login */ }
  }
